...
\begin{document}
	...
	The performance of all classifiers on the training and validation data can be found in tables ~\ref{table:results_train} and ~\ref{table:results_valid} respectively. The best model on the validation data was the CatBoost classifier with 11\% additional accuracy compared to a majority classifier. The results on all evaluation metrics seem balanced, with values ranging from 0.30 to 0.31. The CatBoost classifier is followed  by logistic regression which performs 0.01-0.02 worse on the evaluation metrics. 
	...
	The performance of the final CatBoost classifier on the test data from QTR1 2019 is displayed in table ~\ref{table:results_test}. The corresponding confusion matrix with class-wise precision is shown in table ~\ref{table:best_model_cm_abs}. The performance on the test data is very similar to the performance on the validation data (with differences of only 0.00-0.01). The largest precision is achieved for the class \lstinline{lg_inc}, with a precision of 0.36. This means that when classifying \lstinline{lg_inc}, the model is correct 36\% of the times. The model makes rather many mistakes when classifying \lstinline{sm_inc} and \lstinline{sm_dec}.
...
\bottomrule
	...
	A word cloud with features weighted by feature importance is shown in figure ~\ref{fig:feat_imp}. The feature importance is a normalized CatBoost metric that reflects how much the prediction values change when the feature changes \cite{noauthor_feature_nodate}. Larger values represent larger feature importance.
	
	Overall, there are words that seem to be relevant for stock price changes, e.g. increase, decrease, disappoint, profit, and also words that seem less relevant, e.g. months like April, August, and abbreviations like inc.
	...
	In conclusion, we can say that making use of 8-K filings for stock price predictions has potential for usage in a practical application. A backtesting with simplified assumptions turned out to be profitable. The most important future steps will be a) to try to improve a model by adding different data sources (e.g. stock price time series and fundamental data about the companies) and b) to see if the assumptions made during backtesting hold in actual trades. The CatBoost classifier was shown to be suitable model since it outperformed all other baseline models in a reasonable amount of training time ($< 10$ min. per model). 
